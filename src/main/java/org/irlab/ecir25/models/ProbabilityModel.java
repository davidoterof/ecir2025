...
public final class ProbabilityModel {
  ...
  private final int[] queryNumbers;
  ...
  public ProbabilityModel(Path pathToQrel, MetricEnum metric) {
    ...
    this.queryNumbers = qrels.getTopics().stream().mapToInt(Integer::intValue).toArray();
  ...
  public double[] sampleMetric(double change, int queries, int queriesImproved) {
    int[] sampledQueries = ArrayUtils.subarray(queryNumbers, 0, queries);
    return sampleMetric(change, queries, queriesImproved, sampledQueries);
  }

  public double[] sampleMetric(double change, int queries, int queriesImproved, int[] queryIds) {
    Int2ObjectMap<int[]> sampledRelevants = new Int2ObjectOpenHashMap<>(queries);
    int count = 0;
  ...
  public void learn(int queryId, List<String> ranking, Qrels qrels) {
    ...
    try {
      ...
      LOG.warn("Failed to create regressor for query {} returning default relevants", queryId);
      return;
    }
      ...
      synchronized (probCache) {
        if (probCache.get(queryID).get(cacheKey) == null) {
          ...
          try {
            ...
            // In our experiments, this exception happens when a ranking has no relevants. In that case,
            // the simulated systems will output non-relevants always.
            tmp = relevants.get(queryID);
            probCache.get(queryID).put(cacheKey, tmp);
            LOG.warn("Problems with regressor for query {}", queryID);
