 ...
 * variable. A logit is the natural log of the odds of the dependent equaling a
 * certain value or not (usually 1 in binary logistic models, the highest value
 * in multinomial models). In this way, logistic regression estimates the odds
 * of a certain event (value) occurring.
 * <p>
 * Goodness-of-fit tests such as the likelihood ratio test are available as
 ...
 * thus logistic regression is sometimes called "maximum entropy modeling", and
 * the resulting classifier the "maximum entropy classifier".
 *
 * @author Haifeng Li
 * @see MLP
 * @see Maxent
 * @see LDA
 */
public class CustomLogitLogisticRegressor implements SoftClassifier<double[]>, OnlineClassifier<double[]> {
   ...
   */
  public CustomLogitLogisticRegressor(double[] w, double L, double lambda) {
   ...
   */
  public CustomLogitLogisticRegressor(double L, double[] w, double lambda, IntSet labels) {
    this.p = w.length - 1;
    this.k = 2;
    this.L = L;
    this.w = w;
   ...
   *               especially when the dimensionality is high.
   */
  public CustomLogitLogisticRegressor(double L, double[][] W, double lambda) {
   ...
   */
  public static CustomLogitLogisticRegressor fit(Formula formula, DataFrame data) {
   ...
   * @param data    the data frame of the explanatory and response variables.
   */
  public static CustomLogitLogisticRegressor fit(Formula formula, DataFrame data, Properties prop) {
   ...
   *          binary features. The features are stored in an integer array, of
   *          which are the indices of nonzero features.
   * @param y training labels in [0, k), where k is the number of classes.
   */
  public static CustomLogitLogisticRegressor fit(double[][] x, int[] y) {
   ...
   * @param y training labels in [0, k), where k is the number of classes.
   */
  public static CustomLogitLogisticRegressor fit(double[][] x, int[] y, Properties prop) {
   ...
   */
  public static CustomLogitLogisticRegressor fit(double[][] x, int[] y, double lambda, double tol, int maxIter) {
    if (x.length != y.length) {
      throw new IllegalArgumentException(String.format("The sizes of X and Y don't match: %d != %d",
                                                       x.length,
                                                       y.length));
    }
    ...
    CustomLogitLogisticRegressor model;
    ...
    if (k == 2) {
      ...
      model = new CustomLogitLogisticRegressor(L, w, lambda, codec.labels);
    } else {
  ...
  public static CustomLogitLogisticRegressor dummyAlteredRegressor(double[][] x, int[] y, double[] myw) {
    if (x.length != y.length) {
      throw new IllegalArgumentException(String.format("The sizes of X and Y don't match: %d != %d",
                                                       x.length,
                                                       y.length));
    ...
    CustomLogitLogisticRegressor model;
    if (k == 2) {
      model = new CustomLogitLogisticRegressor(0, myw, lambda, codec.labels);
    } else {
      double[][] W = new double[k - 1][p + 1];
      for (int i = 0, l = 0; i < k - 1; i++) {
        for (int j = 0; j <= p; j++, l++) {
      ...
      model = new CustomLogitLogisticRegressor(0, W, lambda, codec.labels);
    }
    model.setLearningRate(0.1 / x.length);
    return model;
  }
  ...
  /**
   * Sets the learning rate of stochastic gradient descent. It is a good practice
   * to adapt the learning rate for different data sizes. For example, it is
   * typical to set the learning rate to eta/n, where eta is in [0.1, 0.3] and n
   * is the size of the training data.
   *
  ...
  public int predict(double[] x, double[] posteriori) {
    ...
    if (posteriori.length != k) {
      throw new IllegalArgumentException(String.format("Invalid posteriori vector size: %d, expected: %d",
                                                       posteriori.length,
                                                       k));
    }
  ...
  static class BinaryObjectiveFunction implements DifferentiableMultivariateFunction {
     ...
     *
     */
    private static final long serialVersionUID = -3701266197433058631L;
    /**
     * Training instances.
     */
    double[][] x;
  ...
  static class MultiClassObjectiveFunction implements DifferentiableMultivariateFunction {
     ...
     *
     */
    private static final long serialVersionUID = 5581540914605372751L;
    /**
     * Training instances.
     */
    double[][] x;
